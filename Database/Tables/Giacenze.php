	...
	class Giacenze extends Database {
        ...
        public function creaGiacenzeCorrenti() {
            ...
            $sql =" insert into giacenze_correnti
                    ...
                    from $this->tableName as g where g.anno = 2021 and g.data < CURRENT_DATE() and g.negozio not in ('SMBB','SMMD') group by 1,2) as d on g.codice=d.codice and g.negozio=d.negozio and g.data=d.data
            ...
            $sql = "select
                    ...
                    from $this->tableName as g join 
                        (	select g.`codice`, g.`negozio`, max(g.`data`) `data` 
                            from $this->tableName as g where g.anno = 2021 and g.data <= '$dataCalcolo' 
                            group by 1,2
        ...
        public function giacenzeAllaData($dataCalcolo) {
            $sql = "select g.codice, g.negozio, g.giacenza from $this->tableName as g join (select g.`codice`, g.`negozio`, max(g.`data`) `data` 
                    from $this->tableName as g where g.anno = 2021 and g.data <= '$dataCalcolo' group by 1,2) as d on g.codice=d.codice and g.negozio=d.negozio and g.data=d.data
                    order by lpad(SUBSTR(g.negozio,3),2,'0'), g.codice;";
